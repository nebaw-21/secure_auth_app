...
const DisplayEvent = () => {
  ...
  const [galleryInformation, setGalleryInformation] = useState([]);
...
const [data, setData] = useState([]);
  ...
  const deleteOperation = async (id) => {
    try {
      ...
      const response = await axios.delete(`${APP_URL}/gallery/${id}`);
      const result = response.data;
  ...
  async function getData(){
    let response = await axios.get(`${APP_URL}/gallery`);
    const result = response.data;
    setGalleryInformation(result);
   }
  ...
  if (data && data.length > 0) {
    ...
    currentItems = galleryInformation.slice(firstIndex, lastIndex);
  }
  ...
  const pageNumbers = [];
  for (let i = 1; i <= Math.ceil(galleryInformation.length / itemsPerPage); i++) {
    pageNumbers.push(i);
  }
...
<Link to='/createGallery' className=''>
  ...
  <tbody>
    {currentItems.map((gallery, index) => (
        <tr className="hover:bg-gray-100">
          ...
          <td className="py-2 px-4 border-b text-center">{gallery.description}</td>
          <td className="py-2 px-4 border-b text-center">

          <Link to={"/editGallery/"+gallery._id}>
