...
const [data, setData] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${APP_URL}/message`);
  ...
  const deleteOperation = async (id) => {
    try {
      ...
      const response = await axios.delete(`${APP_URL}/message/${id}`);
      const result = response.data;
  ...
  async function getData(){
    let response = await axios.get(`${APP_URL}/message`);
    const result = response.data;
    setMessage(result);
   }
  ...
  if (data && data.length > 0) {
    ...
    currentItems = message.slice(firstIndex, lastIndex);
  }
  ...
  const pageNumbers = [];
  for (let i = 1; i <= Math.ceil(message.length / itemsPerPage); i++) {
    pageNumbers.push(i);
  }
  ...
  <tbody>
    {currentItems.map((message, index) => (
        <tr className="hover:bg-gray-100">
          ...
          <td className="py-2 px-4 border-b text-center">{message.name}</td>
          <td className="py-2 px-4 border-b text-center">{message.emali}</td>
