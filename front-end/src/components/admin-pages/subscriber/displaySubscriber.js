...
const [data, setData] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${APP_URL}/subscribed`);
  ...
  const deleteOperation = async (id) => {
    try {
      ...
      const response = await axios.delete(`${APP_URL}/subscribed/${id}`);
      const result = response.data;
  ...
  async function getData(){
    let response = await axios.get(`${APP_URL}/subscribed`);
    const result = response.data;
    setUserInformation(result);
   }
